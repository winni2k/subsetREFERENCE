...
//        const bool complement = 
	...
	line = 0;
	int found = 0;
	cerr << "Reading haplotype file in [" << argv[2] << "] and writing subset in [" << argv[3] << "]" << endl;
	ifile fdi (argv[2]);
	ofile fdo (argv[3]);
	while (getline(fdi, buffer, '\n')) {
		...
		string a1 = tokens[4];
		bool ok = false;
		pair < multimap < int, snp >::iterator, multimap < int, snp >::iterator > seqM = M.equal_range(pos);
		for ( multimap < int, snp >::iterator it = seqM.first; it != seqM.second && !ok; ++it) 	ok = (ok || it->second.strand(a0, a1));
		if (ok) {
			fdo << buffer << endl;
			found ++;
		}
		if (line % 1000 == 0) { cerr << "\r" << found << " / " << line ; cerr.flush(); }
		line ++;
	}
